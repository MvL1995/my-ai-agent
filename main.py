...
web_design_agent = Agent(
    ...
    instructions=(
        ...
        "用户路径、响应式要求和验收标准。"
        "Landing Page 必须包含公开、无需登录的项目咨询表单与预约30分钟入口；"
        "无需登录不等于无身份字段，姓名和邮箱仍必填。"
        "表单覆盖姓名、邮箱、公司、咨询或预约意向、预约时间和留言，"
        "表单字段仅限 name、email、company、intent、preferred_time、"
        "message 与反垃圾 website；不要求埋点、时区或备选时间。"
        "并明确提交成功、失败状态及基础无障碍要求。"
...
coding_agent = Agent(
    ...
    instructions=(
        ...
        "页面必须包含 id=lead-form 的无需登录表单，字段 name、email、company、"
        "intent、preferred_time、message 和隐藏蜜罐 website；"
        "不得新增其他表单字段，也不要新增埋点；"
        "preferred_time 使用原生 datetime-local。"
        "intent 只能使用 value=project 或 value=booking；booking 必须填写预约时间。"
        "预约30分钟入口用 data-lead-intent=booking 指向该表单。"
        "表单必须包含 type=submit 按钮及 role=status、aria-live 状态区域。"
        "script.js 必须为空字符串；平台注入固定线索提交脚本，"
        "负责 iframe 消息、独立 API 请求和提交状态。"
        "index.html 只能引用一次 script.js，不得包含其他 script 标签。"
...
qa_agent = Agent(
    ...
    instructions=(
        ...
        "使用中文，第一行必须严格输出“结论：通过”或“结论：需修改”。"
        "随后输出验收覆盖情况、"
        "关键问题和可执行修改建议。"
        "只检查已提供的内容；缺少证据时标记为未验证。"
        "Landing Page 若缺少无需登录的咨询表单、预约30分钟入口、"
        "name/email/company/intent/preferred_time/message/website 字段、"
        "type=submit 按钮、role=status 与 aria-live 状态区域"
        "或基础无障碍支持，结论必须为“需修改”。"
        "无需登录仅指无需账户，name 和 email 仍可必填。"
        "同一表单中的 booking 意向与预约时间即为有效预约入口，"
        "不要求外部日历服务。平台注入固定线索提交脚本，"
        "负责 iframe 消息、独立 POST /api/leads 与预约时间必填状态；"
        "script.js 为空是正确交付，不得据此阻断。"
        "index.html 必须只引用一次 script.js，其他 script 标签属于阻断项。"
        "不得仅因尚未发布或未进行真实流量测试而判需修改，应标为未验证。"
        "阻断项仅限上述平台契约与原始项目硬要求；"
        "埋点、时区、备选时间等增强建议不属于阻断项。"
        "JavaScript 动态设置 required 视为已实现；"
        "代码可判定的行为不得仅因未运行而判失败。"
